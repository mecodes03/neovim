...
local function get_bundles()
	...
	local java_test_jars = vim.fn.glob(java_test_path .. "/extension/server/*.jar", 1)
...
local function setup_jdtls()
	...
	local cmd = {
		...
		'-Dlog.protocol=true', -- for debug
		'-Dlog.level=ALL', -- for debug
	...
	local on_attach = function(client, bufnr)
		-- Disable semantic tokens so LSP doesn't override treesitter highlights,
		-- then force-stop the engine and re-trigger treesitter highlighting
